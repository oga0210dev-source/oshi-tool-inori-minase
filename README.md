...
* 無料での運用を前提とする
* 必要な業務データのみ保存する
* 個別の操作履歴・詳細なアクセスログは保存しない
* サイト利用状況の集計に必要なアクセス集計データは保存する
* ユーザー登録は常時可能とする
* 招待コードによる登録制限は設けない
...
`announcement_id` はメンテナンスに紐付く
`m_announcement.announcement_id` を参照する。

通常メンテナンスでは、メンテナンス告知を事前に公開する。

通常メンテナンスの開始には、
紐付けられたメンテナンス告知が公開されてから
24時間以上経過していることを必要とする。

緊急メンテナンス（`is_emergency = TRUE`）の場合は、
24時間ルールの対象外とする。

`is_active` によりメンテナンスの有効状態を管理する。

* `TRUE`：有効
* `FALSE`：無効

現在の管理画面では、メンテナンスタイプは
サービス全体（`ALL`）で固定して運用する。

将来的な拡張に備えて `PARTIAL` および `target_key` は
DB・Model・Routerに保持する。
...
現時点では一部機能メンテナンスは運用しない。

メンテナンス中は、管理者を除くユーザー・ゲストを
メンテナンス画面へ誘導する。

管理者はメンテナンス中もサービスを利用できる。
...
`capacity` は会場の収容人数を管理する。

`official_url` は会場公式ホームページへのリンクを管理する。
座席表・座席画像等の確認に使用する。
...
アクセス日は日本時間（JST）の**6:00区切り**で判定する。
...
* 個別の操作履歴・詳細なアクセスログは実装しない
* アクセス数を確認するための集計データは必要最小限の範囲で保存する
