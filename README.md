...
予測セトリで選択できる楽曲は、LIVEのセトリ（`m_setlist.event_type = 'LIVE'`）に
1件以上登録されている楽曲を対象とする。

町民集会（`event_type = 'CHOMIN'`）でのみ使用されている楽曲は、
セトリ予測の選択対象から除外する。

同一楽曲名の `m_song` レコードが複数存在する場合、
セトリ予測の楽曲選択画面では同一楽曲名を1件にまとめて表示する。

町民集会でも使用されている楽曲であっても、
LIVEのセトリに登録されている場合はセトリ予測の対象とする。
